...
def get_arxiv_paper_from_web(query: str, start: datetime.datetime, end: datetime.datetime) -> list[arxiv.Result]:
    ...
    all_paper_ids = []
    for cat in cats:
        url = f"https://arxiv.org/list/{cat}/new"  # 仅获取昨天提交的最新论文
...
def get_biorxiv_papers(start_date: str, end_date: str, query: str = "") -> list:
    """
    从 bioRxiv 获取在指定日期范围内的预印本论文。
    ...
    if response.status_code != 200:
        logger.warning("无法获取数据: {}".format(response.status_code))
        return []
...
def send_email(sender: str, receiver: str, password: str, smtp_server: str, smtp_port: int, html: str, ):
    ...
    msg['Subject'] = Header(f'Daily arXiv & bioRxiv {today}', 'utf-8').encode()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--biorxiv_query', type=str, help='bioRxiv search query', default=os.environ.get('BIORXIV_QUERY', ""))
    ...
    arxiv_papers = get_arxiv_paper(args.arxiv_query, yesterday, today, args.debug)

    logger.info("Retrieving bioRxiv papers...")
    biorxiv_papers = get_biorxiv_papers(yesterday.strftime('%Y-%m-%d'), today.strftime('%Y-%m-%d'), args.biorxiv_query)

    # 合并 arXiv 和 bioRxiv 论文
    all_papers = arxiv_papers + biorxiv_papers

    if len(all_papers) == 0:
        logger.info("No new papers found. Maybe a holiday and no one submitted their work. If this is not the case, please check the queries.")
        logger.info("No email will be sent. Enjoy a relaxing day!")
        exit(0)

    logger.info("Reranking papers...")
    all_papers = rerank_paper(all_papers, corpus)
    
    if args.max_paper_num != -1:
        all_papers = all_papers[:args.max_paper_num]
